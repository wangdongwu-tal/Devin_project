...
async function createMockQuoteResponse(index: number): Promise<any> {
...
async function runTest() {
    ...
    try {
        ...
        const consumer = await createConsumer(RABBITMQ_URL, QUEUE_NAME, async (message: any) => {
        ...
        const producers = await Promise.all(
            Array(producerCount).fill(0).map(async (_, index) => {
